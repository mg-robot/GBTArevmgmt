...
export default class PromoCodeWizardStepDefinition extends LightningElement {
  ...
  get hasType() {
    return !!this._localData.discountType;
  }
  get isPercent() {
    return this._localData.discountType === "Percent";
  }
  get isAmount() {
    return this._localData.discountType === "Amount";
  }
  get currencyOptions() {
    return (this.availableCurrencies || []).map((c) => ({
      label: c,
      value: c
    }));
  }
  get hasCodeConflicts() {
    return this.codeConflicts && this.codeConflicts.length > 0;
  }
  get codeConflictsLabel() {
    return this.codeConflicts ? this.codeConflicts.join(", ") : "";
  }
  get d() {
  ...
  renderedCallback() {
    if (this._initialized && this.wizardData) {
      ...
      for (const k of Object.keys(this.wizardData)) {
        const localVal = this._localData[k];
        const wizardVal = this.wizardData[k];
        // Resync if key is absent, OR if an empty-array default was set before an
        // async parent update (e.g. percentCurrencies populated after getActiveCurrencies resolves).
        const emptyArrayStale =
          Array.isArray(localVal) &&
          localVal.length === 0 &&
          Array.isArray(wizardVal) &&
          wizardVal.length > 0;
        if (
          (localVal === undefined || emptyArrayStale) &&
          wizardVal !== undefined
        ) {
          this._localData = { ...this._localData, [k]: wizardVal };
          resynced = true;
        }
      }
    ...
    if (upper.length >= 2) {
      ...
      this._debounceTimer = setTimeout(() => {
        checkCodeUniqueness({ code: upper })
          .then((r) => {
            this.codeConflicts = r && r.length ? r : null;
          })
          .catch(() => {
            this.codeConflicts = null;
          });
  ...
  handleDisplayNameChange(e) {
    this.dispatch("displayName", e.target.value);
  }
  handleTermsChange(e) {
    this.dispatch("terms", e.target.value);
  }
  handleDiscountTypeChange(e) {
    this.dispatch("discountType", e.detail.value);
  }
  handlePercentValueChange(e) {
    const v = e.target.value;
    this.dispatch("percentValue", v === "" || v == null ? null : parseFloat(v));
  }
  handlePercentCurrenciesChange(e) {
    this.dispatch("percentCurrencies", e.detail.value);
  }
  ...
  fireValidate() {
    this.dispatchEvent(
      new CustomEvent("stepvalidate", {
        detail: { valid: this.computeValid() }
      })
    );
  }
  ...
  computeValid() {
    ...
    if (d.discountType === "Percent") {
      if (d.percentValue == null || d.percentValue <= 0 || d.percentValue > 100)
        return false;
      if (!d.percentCurrencies || d.percentCurrencies.length === 0)
        return false;
    } else if (d.discountType === "Amount") {
      if (!d.amounts || d.amounts.length === 0) return false;
      if (d.amounts.some((a) => !a.currencyIsoCode || !(a.amount > 0)))
        return false;
