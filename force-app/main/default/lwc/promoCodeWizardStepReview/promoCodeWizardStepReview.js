...
export default class PromoCodeWizardStepReview extends LightningElement {
  ...
  get isPercent() {
    return this.wizardData?.discountType === "Percent";
  }
  get isAmount() {
    return this.wizardData?.discountType === "Amount";
  }
  ...
  get previewText() {
    return LBL_PreviewTemplate.replace("{0}", this.recordCount).replace(
      "{1}",
  ...
  get discountValueDisplay() {
    ...
    if (!d) return "";
    if (this.isPercent)
      return `${d.percentValue}% (${(d.percentCurrencies || []).join(", ")})`;
    if (this.isAmount) {
      return (d.amounts || [])
  ...
  get applicableToDisplay() {
    return this.wizardData?.applicableTo || "Both";
  }
  get minMonthsDisplay() {
    const v = this.wizardData?.minMonthsSinceLastActive;
    if (v == null || v === "") return LBL_None;
    return `${v} months`;
  }
  get showMinMonths() {
  ...
  get totalLimitDisplay() {
    return this.wizardData?.totalLimit == null
      ? LBL_Unlimited
      : String(this.wizardData.totalLimit);
  }
  get perMemberLimitDisplay() {
    return this.wizardData?.perMemberLimit == null
      ? LBL_Unlimited
      : String(this.wizardData.perMemberLimit);
  }
  get combinableDisplay() {
    if (!this.wizardData?.combinable) return LBL_No;
    return LBL_CombinableYesGroup.replace(
      "{0}",
