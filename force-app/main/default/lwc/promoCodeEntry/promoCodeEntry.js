...
export default class PromoCodeEntry extends LightningElement {
    ...
    get totalAdjustment() {
        return this.appliedCodes.reduce((sum, c) => sum + (Number(c.appliedAmount) || 0), 0);
    }
    ...
    handleRemove(event) {
        ...
        if (remaining.length === 0) {
            ...
            this.dispatchEvent(new CustomEvent('codesremoved', {
    ...
    async _callApply(codes, { isAdd = false, isRemove = false } = {}) {
        ...
        try {
            const previousGroupId = this.applicationGroupId;
            const result = await applyCodes({ orderId: this.orderId, codes });
            ...
            if (applied.length > 0) {
                this.dispatchEvent(new CustomEvent('codesapplied', {
                    detail: {
