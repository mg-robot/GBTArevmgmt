...
jest.mock(
  "@salesforce/apex/PromoCodeApplyService.getCurrentlyApplied",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
...
describe("c-promo-code-entry", () => {
  beforeEach(() => {
    // Default: no codes currently applied. Tests that need to exercise the auto-load path
    // can override via getCurrentlyApplied.mockResolvedValueOnce(...) within the test body.
    getCurrentlyApplied.mockResolvedValue({
      codeResults: [],
      totalAdjustment: 0,
  ...
  test("invalid code renders red error banner + dispatches validationerror", async () => {
    ...
    expect(onError.mock.calls[0][0].detail.codeResults[0].errorCode).toBe(
      "NOT_FOUND"
    );
  ...
  test("removing an applied code re-applies with smaller list + dispatches codesremoved", async () => {
    ...
    const removeIcons = el.shadowRoot.querySelectorAll(
      "lightning-button-icon[data-code]"
    );
  ...
  test("removing the last applied code skips Apex + still dispatches codesremoved", async () => {
    ...
    const removeBtn = el.shadowRoot.querySelector(
      'lightning-button-icon[data-code="ONLY"]'
    );
    removeBtn.click();
    await flushPromises();
