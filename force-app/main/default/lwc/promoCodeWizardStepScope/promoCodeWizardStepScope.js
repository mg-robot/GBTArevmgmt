...
export default class PromoCodeWizardStepScope extends LightningElement {
  ...
  @wire(getPicklistValues, {
    recordTypeId: MASTER_RT,
    fieldApiName: PRODUCT_SCOPE_TYPE_FIELD
  })
  wiredProductScopeType({ data }) {
    if (data)
      this._productScopeTypeOptions = data.values.map((v) => ({
        label: v.label,
        value: v.value
      }));
  }
  @wire(getPicklistValues, {
    recordTypeId: MASTER_RT,
    fieldApiName: PRODUCT_FAMILY_SCOPE_FIELD
  })
  wiredProductFamily({ data }) {
    if (data)
      this._productFamilyOptions = data.values.map((v) => ({
        label: v.label,
        value: v.value
      }));
  }
  @wire(getPicklistValues, {
    recordTypeId: MASTER_RT,
    fieldApiName: APPLICATION_LEVEL_FIELD
  })
  wiredApplicationLevel({ data }) {
    if (data)
      this._applicationLevelOptions = data.values.map((v) => ({
        label: v.label,
        value: v.value
      }));
  }
  ...
  get productScopeTypeOptions() {
    return this._productScopeTypeOptions;
  }
  get productFamilyOptions() {
    return this._productFamilyOptions;
  }
  get applicationLevelOptions() {
    return this._applicationLevelOptions;
  }
  ...
  handleProductScopeTypeChange(e) {
    ...
    if (prev && prev !== v) {
      ...
      if (prev === "Specific Membership Types")
        this.dispatch("memberTypeScope", []);
      if (prev === "Specific Products") this.dispatch("specificProducts", "");
    }
  }

  handleProductFamilyScopeChange(e) {
    this.dispatch("productFamilyScope", e.detail.value);
  }
  handleMemberTypeScopeChange(e) {
    this.dispatch("memberTypeScope", e.detail.value);
  }
  handleSpecificProductsChange(e) {
    this.dispatch("specificProducts", e.detail.csv);
  }
  ...
  fireValidate() {
    ...
    if (
      d.productScopeType === "Product Family" &&
      (!d.productFamilyScope || d.productFamilyScope.length === 0)
    )
      valid = false;
    if (
      ...
      valid = false;
    if (d.productScopeType === "Specific Products" && !d.specificProducts)
      valid = false;
